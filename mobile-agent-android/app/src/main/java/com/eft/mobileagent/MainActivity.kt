...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        behaviorQueueRepository = BehaviorQueueRepository(this)
        behaviorConfigStore = BehaviorAgentConfigStore(this)

        datePicker = findViewById(R.id.datePicker)
        timePicker = findViewById(R.id.timePicker)
        timePicker.setIs24HourView(true)
        backendBaseUrlInput = findViewById(R.id.backendBaseUrlInput)
        syncUserIdInput = findViewById(R.id.syncUserIdInput)
        behaviorAccessTokenInput = findViewById(R.id.behaviorAccessTokenInput)
        loginSyncUserButton = findViewById(R.id.loginSyncUserButton)
        loginFormContainer = findViewById(R.id.loginFormContainer)
        editLoginButton = findViewById(R.id.editLoginButton)
        loggedInUserStatusView = findViewById(R.id.loggedInUserStatusView)
        syncServerAlarmsButton = findViewById(R.id.syncServerAlarmsButton)
        behaviorStartButton = findViewById(R.id.behaviorStartButton)
        behaviorStopButton = findViewById(R.id.behaviorStopButton)
        behaviorFlushButton = findViewById(R.id.behaviorFlushButton)
        behaviorStatusView = findViewById(R.id.behaviorStatusView)
        behaviorRefreshQuestionButton = findViewById(R.id.behaviorRefreshQuestionButton)
        behaviorQuestionTextView = findViewById(R.id.behaviorQuestionTextView)
        behaviorAnswerWorkButton = findViewById(R.id.behaviorAnswerWorkButton)
        behaviorAnswerRestButton = findViewById(R.id.behaviorAnswerRestButton)
        behaviorAnswerMoveButton = findViewById(R.id.behaviorAnswerMoveButton)
        ...
        sourceGoogleRadio = findViewById(R.id.sourceGoogleRadio)
        missionTypeGroup = findViewById(R.id.missionTypeGroup)
        missionLocationRadio = findViewById(R.id.missionLocationRadio)
        missionManualRadio = findViewById(R.id.missionManualRadio)
        ...
        findViewById<Button>(R.id.saveCurrentLocationButton).setOnClickListener {
            ensureLocationPermissionThenSaveTarget()
        }
        findViewById<Button>(R.id.scheduleAlarmButton).setOnClickListener {
            scheduleAlarm()
        }
    ...
    private fun startBehaviorQuestionPolling() {
        ...
        behaviorQuestionPollRunnable = object : Runnable {
            override fun run() {
                if (!behaviorQuestionPollingActive || isFinishing) return
                refreshPendingBehaviorQuestion(manual = false)
    ...
    private fun restoreSyncInputs() {
        ...
        if (config != null) {
            ...
            ReminderSyncWorkScheduler.ensurePeriodicSync(this)
            ReminderSyncWorkScheduler.triggerImmediateSync(this)
        }
    }
    ...
    private fun syncServerReminders() {
        val savedConfig = ReminderSyncManager.loadConfig(this)
        if (savedConfig == null) {
    ...
    private fun loginSyncUser() {
        ...
        backendBaseUrlInput.setText(baseUrl)
        val previousUserId = ReminderSyncManager.loadConfig(this)?.userId

        loginSyncUserButton.isEnabled = false
        Thread {
            val result = runCatching {
                val user = ReminderSyncClient(baseUrl).login(identifier)
                ReminderSyncManager.saveConfig(this, baseUrl, user.userId)
                ReminderSyncWorkScheduler.ensurePeriodicSync(this)
                ReminderSyncWorkScheduler.triggerImmediateSync(this)
            ...
            runOnUiThread {
                ...
                result.onSuccess { (user, syncResult) ->
                    ...
                    BehaviorAgentController.start(this)
                    refreshBehaviorStatusUi()
    ...
    private fun startBehaviorAgent() {
        ...
        ReminderSyncManager.saveConfig(this, inputs.baseUrl, inputs.userId)
    ...
    private fun openRecoveryUrl(url: String) {
        try {
            ...
            toast("웹 복구 페이지 열기 실패: ${e.message ?: "unknown"}")
        }
    }
    ...
    private fun refreshBehaviorQuestionUi() {
        ...
        behaviorAnswerRestButton.text = if (isSoftNudge) "잠깐 막힘 있어요" else "rest"
