  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        {/* 4:3 종횡비 컨테이너 */}
        <div className="relative w-full max-w-2xl mx-auto">
          <div className="relative w-full aspect-[4/3] rounded-lg shadow overflow-hidden bg-black">
            <video
              ref={videoRef}
              className="absolute inset-0 w-full h-full object-cover"
              autoPlay
            ...
            {showIntro && (
              <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center">
                ...
                {/* 🔹 시작 버튼 */}
                <button
                  className="mt-6 px-8 py-3 bg-emerald-500 hover:bg-emerald-600 
                            text-white font-bold rounded-lg text-lg shadow-lg"
                  onClick={() => {
                    setShowIntro(false);
                    (window as any).__startEFTGuide?.();
                  }}
      ...
      {/* 🔹 인트로 전체 덮는 오버레이 (화면 전체 + 흐림 + 딤 처리) */}
      {showIntro && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center">
          <div className="max-w-2xl w-full px-4">
            <div className="bg-black/80 rounded-xl p-4 shadow-xl flex flex-col items-center">
              <img
                src={introImg}
                alt="Moodtoc Game Intro"
              ...
              <button
                className="mt-4 px-6 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-lg text-sm"
                onClick={() => {
                  setShowIntro(false);
                  (window as any).__startEFTGuide?.();
                }}
...
// 여기서 지금 뒤에 오버레이블랙으로 하는 거 다시 하기 ! 이거 이전으로 백업한걸 ARHolisticTest copy 이미지는 뜨는데, 뒤에 검정배경안됨이거랑 동일상황
