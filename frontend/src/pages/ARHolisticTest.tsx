      ...
      try {
        ...
        holistic.onResults((res) => {
          ...
          if (currentGuide.running && currentGuide.phase === "tapping") {
            ...
            // 💡 현재 가이드 포인트 위치에 두더지 띄우기 (0~1 비율로 저장)
            // 성능 주의: onResults가 프레임마다 불려서 setState가 좀 많아질 수는 있는데,
            // MVP 시연용으로는 이 정도는 충분히 돌아갈 거야.
            // 나중에 느리면 useRef + style.transform 직접 업데이트로 최적화하면 됨
            if (curPt) {
              setMolePos({
                x: curPt.x / c.width,
                y: curPt.y / c.height,
            ...
            if (curPt && (leftHand || rightHand)) {
              ...
              if (hit && nowTap - lastTapTimeRef.current > TAP_COOLDOWN_MS) {
                ...
                // 🕳 여기부터: 손이 닿으면 두더지 들어가는 애니메이션
                if (moleHideTimeoutRef.current) {
                  window.clearTimeout(moleHideTimeoutRef.current);
                  moleHideTimeoutRef.current = null;
                }

                // down1~3 프레임 인덱스 (MOLE_FRAMES 기준)
                const downFrames = [6, 7, 8]; // down1, down2, down3
                const FRAME_INTERVAL = 80;    // ms - 속도는 기호대로 60~120 조절

                downFrames.forEach((frame, i) => {
                  window.setTimeout(() => {
                    setMoleFrame(frame);
                    // 마지막 프레임이면 살짝 있다가 숨기기
                    if (i === downFrames.length - 1) {
                      moleHideTimeoutRef.current = window.setTimeout(() => {
                        setMoleActive(false); // 구멍 속으로 완전히 숨기기
                      }, 40);
                    }
                  }, i * FRAME_INTERVAL);
                });
            ...
            {moleActive && molePos && (
              ...
              <img
                ...
                style={{
                  ...
                  left: `${molePos.x * 100}%`,
                  top: `${molePos.y * 100}%`,
                  // 두더지가 "구멍 위에 서 있는 느낌" 나게 Y 살짝 올리기
                  transform: "translate(-50%, -80%)",
                  width: 128,
                  height: 119,
                  pointerEvents: "none",
                  imageRendering: "pixelated", // 픽셀레트로 느낌 유지
                  zIndex: 25,
                }}
              />
