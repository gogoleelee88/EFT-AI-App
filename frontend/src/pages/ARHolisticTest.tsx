...
const OFF = {
  EB: { dx: 0, dy: -0.010 }, // 원래 위치로 (좌우 이동 없음)
  UE: { dx: -0.010, dy:  0.035 }, // 왼쪽/아래로 살짝
};
      ...
      try {
        ...
        const drawOverlay = (t: number) => {
          ...
          allPoints.forEach(({key, label, color}) => {
            const isCurrentPoint = currentPoint === key;
          ...
          if (currentPoint) {
            const pointText = `현재 포인트: ${currentPoint}`;
            ctx.save();
