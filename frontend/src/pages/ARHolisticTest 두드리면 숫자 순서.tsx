    ...
    const setup = async () => {
      try {
            ...
            if (curPt && (leftHand || rightHand)) {
              ...
              if (hit && nowTap - lastTapTimeRef.current > TAP_COOLDOWN_MS) {
                ...
                if (moodScoreRef.current > 0) {
                  ...
                  // 버블 팝 시작 시점 갱신 → drawOverlay에서 애니메이션
                  ...
                  // 0이 되면 다음 포인트/단계로 넘어가도록 deadline 당기기
                  // if (next <= 0) {
        ...
        const drawOverlay = (t: number) => {
          ...
          // 가이드가 돌고 있으면 SEQUENCE 기준, 아니면 기존 currentPoint 기준
          const activePointId: StepId | null = guideActiveId ?? currentPoint ?? null;
