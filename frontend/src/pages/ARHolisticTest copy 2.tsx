...
  ctx.clearRect(0, 0, c.width, c.height);
  ...
  // -----------------------------
  if (showFramingGuide && !guideEngineRef.current.running) {
    const cx = c.width / 2;
    const cy = c.height * 0.45; // 약간 상단에 위치

    // 사용자가 올바른 위치에 있으면 초록색, 아니면 반투명 흰색
    ctx.save();
    ctx.strokeStyle = isPositionCorrect ? "rgba(0, 255, 150, 0.8)" : "rgba(255, 255, 255, 0.4)";
    ctx.lineWidth = 5;
    ctx.setLineDash([15, 10]); // 점선 효과 (사람 테두리 느낌)
