  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      try {
                ...
                if (moodScoreRef.current > 0) {
                  ...
                  if (next <= 0) {
                    ...
                    guideEngineRef.current.deadlineMs = nowTap;
                                  ...
                                  // 🗺 캔버스 좌표(curPt)를 화면 좌표로 변환해서
                  // HTML DOM 이펙트("톡!" + 파티클)를 같은 위치에 뿌리기
        ...
        // 🔍 [PATCH START] drawOverlay 전체 교체
const drawOverlay = (t: number) => {
  pulseRef.current = t;
  const now = performance.now();

  // ⚠️ 캔버스 오버레이 초기화 (비디오 위에 그리는 레이어만 지움)
  ctx.clearRect(0, 0, c.width, c.height);

  const eng = guideEngineRef.current;
  let activePointId: StepId | null = null;

  // 1) 가이드 엔진 상태 업데이트 (탭핑/호흡 단계 전환)
  if (eng.running) {
    if (eng.phase === "tapping") {
      ...
      // 현재 활성 포인트 ID 저장 → 밑에서 포인트 하이라이트에 사용
      activePointId = cur.id;
      ...
      if (remainMs <= 0) {
        ...
        if (nextStep < SEQUENCE.length) {
          // 다음 포인트로 이동
          const nextStepConf = SEQUENCE[nextStep];
          guideEngineRef.current = {
            ...eng,
            stepIdx: nextStep,
            deadlineMs: now + nextStepConf.seconds * 1000,
          };
          setStepIdx(nextStep);
          setStepRemaining(nextStepConf.seconds);
          console.log(
            `라운드 ${eng.round + 1}/${TAPPING_ROUNDS} - ${nextStepConf.label_ko} 시작`
          );
          playBeep(880, 150);
          vibrate(100);
        } else {
          // 라운드 종료 → 다음 라운드 또는 호흡 단계로 전환
      ...
      // 2) 호흡 단계 비주얼 렌더링 (원/bubble 애니메이션)
      ...
      const radius =
        ...
        (BREATH_RADIUS_MAX - BREATH_RADIUS_MIN) *
          (eng.breathPart === "EXHALE" ? 1 - eased : eased);
      ...
      // ⏱ 호흡 타임라인 전환 로직 (남은 시간이 다 되면 다음 파트로)
      if (remainMs <= 0) {
        if (eng.breathPart === "INHALE") {
          guideEngineRef.current = {
            ...eng,
  ...
  // 3) 공통 오버레이 (포인트 버블, 감정 배지, 안내 문구, affirm 텍스트)

  // 👉 가이드가 돌고 있으면 activePointId 하나만 하이라이트
  // 👉 가이드가 안 돌면(activePointId 없음) 모든 포인트를 그려줌
  const guideActiveId: StepId | null = activePointId;
  ...
  allPoints.forEach(({ key, label, color }) => {
    // 가이드 모드일 때(activePoint 존재) → 그 포인트만 그리기
    if (isGuidedMode && guideActiveId && key !== guideActiveId) return;
    const isCurrentPoint = !!guideActiveId && guideActiveId === key;
  ...
  // 🎯 하단 "여기를 가볍게 두드려 주세요" 안내 문구
  if (guideActiveId) {
    const meta = SEQUENCE.find((s) => s.id === guideActiveId);
    const labelKo = meta?.label_ko ?? guideActiveId;
    const labelEn = meta?.label_en ?? guideActiveId;

    const textKo = `여기를 가볍게 두드려 주세요: ${labelKo}`;
    const textEn = `Tap gently here: ${labelEn}`;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-3 flex items-center justify-center gap-2">
          <button
            ...
            title={
              lang === "ko"
                ? "가이드 수동 시작(타이머는 아래 버튼)"
                : "Manual guide start (timer below)"
            }
          >
            {texts.highlight}
          </button>
