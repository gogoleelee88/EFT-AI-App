...
export function ActionRunner({ actions }: { actions: any[] }) {
  ...
  React.useEffect(() => {
    ...
    const hasAsk = acts.some((a) => a.type === "ask_suds");
    const hasSuggest = acts.some((a) => a.type === "suggest_eft");
    if (hasAsk || hasSuggest) {
      // eslint-disable-next-line no-console
      console.log(
        `✅ actions received${hasAsk ? " → ask_suds ready" : ""}${hasSuggest ? " → suggest_eft ready" : ""}`
      );
    }
  }, [acts]);
  ...
  React.useEffect(() => {
    ...
    // eslint-disable-next-line no-console
    console.log("✅ route change triggered from action", immediate);
    nav(
