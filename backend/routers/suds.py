...
def _get_supabase():
    ...
    key = os.getenv("SUPABASE_SERVICE_ROLE_KEY")  # ê¶Œì¥ (RLS ?í–¥ ìµœì†Œ)
...
async def save_suds(request: SUDSRequest, access_token: Optional[str] = None) -> SUDSResponse:
    ...
    # ???¬ì „(STRICT7) ì¡°íšŒ ??Notion ?¤í™(STRICT6)ë¡?ë³€????Notion ?˜ì´ì§€ ?ì„±
    try:
        sb = _get_supabase()
        checkin_res = (
            sb.table("emotion_checkins")
        ...
        delta = intensity_before - intensity_after  # Notion ?ë¬ë–Ÿ?ë¨?£„ ?¨ê¾©ê¶??

        # user_email?€ Notion ?”ì²­?ì„œ ?„ìˆ˜
        user_email = (checkin.get("user_id") or request.user_id or "")
        if "@" not in user_email:
            user_email = "unknown@example.com"  # ???„ì‹œ(?ŒìŠ¤?¸ìš©). ?˜ì¤‘???„ë¡ ?¸ì—???´ë©”??ë³´ë‚´ë©??¬ê¸° ?œê±° ê°€??
        ...
        notion_body = {
            ...
            "strict_intake": {
                ...
                # Notion ëª¨ë¸??ë°›ëŠ” STRICT6 ?„ë“œ?¸ë° ì§€ê¸?DB???†ìœ¼ë©?None?¼ë¡œ
