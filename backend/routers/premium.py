...
async def call_vllm_direct(message: str, temperature: float = 0.7, max_tokens: int = 700) -> dict:
    ...
    if client is None:
        return {
            "response": "?„ì¬ AI ?œë¹„?¤ì— ?°ê²°?????†ìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„??ì£¼ì„¸??",
    ...
    try:
        ...
        if not response.choices:
            return {
                "response": "AI ?‘ë‹µ???ì„±?˜ì? ëª»í–ˆ?µë‹ˆ??",
                "model": model,
                "success": False,
            }
        ...
        if not response_text:
            return {
                "response": "AI ?‘ë‹µ??ë¹„ì–´ ?ˆìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„??ì£¼ì„¸??",
                "model": model,
                "success": False,
            }
        ...
        return {
            "response": "?¤ì‹œê°?AI ?‘ë‹µ ?¸ì¶œ???¤íŒ¨?ˆìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„??ì£¼ì„¸??",
            "model": "error",
            "success": False,
        }
...
async def chat(
    ...
    try:
        ...
        if not result["success"]:
            return ChatResponse(
                response=(
                    "ë¬¸ì œ ?†ì´ ?´ì–´ê°????ˆë„ë¡??„ì??œë¦´ê²Œìš”. "
                    "ê°ì •???ˆë¬´ ê³¼ë??˜ì²˜???ê»´ì§??ŒëŠ” 30ë¶??•ë„ ?´ì‹ ?? "
                    "?¨ì„ ì²œì²œ??ê³ ë¥´ê³??¤ì‹œ ë§ì???ì£¼ì„¸??"
                ),
